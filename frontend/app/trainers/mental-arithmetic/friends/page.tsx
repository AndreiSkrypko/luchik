...
function generateSession(settings: FriendsSettings): FriendsSession {
  ...
  for (let i = 0; i < settings.count; i++) {
    ...
    if (settings.digitRange === '1..10') {
      ...
      // Генерируем число от 10 до 100, заканчивающееся на друга
      ...
      // Генерируем число от 100 до 1000, заканчивающееся на друга
      ...
      // Генерируем число от 1000 до 10000, заканчивающееся на друга
...
export default function FriendsTrainerPage() {
  ...
  const friendDescription = useMemo(() => {
    const option = FRIEND_OPTIONS.find((opt) => opt.value === formState.friend);
    return option?.label ?? FRIEND_OPTIONS[0].label;
  }, [formState.friend]);
